...
\hfill
\begin{minipage}{0.7\textwidth}
    L'\textbf{ensemble vide} $ \emptyset = \{\}$ a 0 élément.
\end{minipage}
\vspace{0.5cm}
\hrule
\vspace{0.5cm}
...
\begin{minipage}{0.3\textwidth}
    \begin{tikzpicture}[scale=0.7]
        ...
        \node at (1.1,0) {$\overline{A}$};
    \end{tikzpicture}
\end{minipage}%
\hfill
\begin{minipage}{0.7\textwidth}
    $A^c$ ou $\overline{A}$, l'ensemble \textbf{complémentaire} de $A$ dans $\Omega$.\\
    \ex{\begin{eleve}$\Omega =$ les élèves de la classe.\\
    $A =$ les élèves de la classe ayant leur anniversaire en juillet.\\
    $\overline{A} =$ les élèves de la classe n'ayant pas leur anniversaire en juillet.\\\end{eleve}}
\end{minipage}
\vspace{0.5cm}
\hrule
\vspace{0.5cm}
...
\begin{enumerate}
    ...
    \vspace{10pt}
    \item $\Omega =$ l'école Christ-Roi. $A =$ la salle de gym.\\ 
    \vspace{10pt}
    Une mesure : \begin{eleve}surface en mètres carrés.\end{eleve}\\ 
    \vspace{10pt}
    Une autre mesure : \begin{eleve}capacité maximale en nombre de personnes.\end{eleve}
   \vspace{10pt}
    \item $\Omega =$ tous les aliments de mon sac de courses. 
    $A =$ les pommes dans mon sac.\\ 
    \vspace{10pt}
    Une mesure : \begin{eleve}masse en kg.\end{eleve}\\ 
    \vspace{10pt}
    Une autre mesure : \begin{eleve}prix en euros.\end{eleve}\\ 
    \vspace{10pt}
    Une autre mesure : \begin{eleve}nombre d'objets.\end{eleve}
...
\begin{tcolorbox}[colback=cyan!0, colframe=cyan!70!blue, title=Définition d'une mesure]
...
\begin{enumerate}
    ...
    \item \textbf{Additivité dénombrable} : {\color{gray} si $(A_n)_{n \ge 1}$ est une suite d’ensembles disjoints deux à deux, alors
        \[
...
\end{tcolorbox}
   ...
   ($A \cap B = \emptyset$ : personne ne peut être né 
   en même temps en mars et en juillet).\\
   $A \cup B =$ Les élèves nés en juillet ou en mars, $\qquad \mu(A \cup B) =$}
\newpage
    \begin{tcolorbox}[colback=magenta!0, colframe=red!50!blue, title=Propriétés des ensembles complémentaires]
    \prop{Si $A\subset\Omega$, et $\overline{A} = \Omega \setminus A $ le complémentaire de $A$ dans $\Omega$. \\Par définition, $A$ et $\overline{A}$ sont disjoints et $A\cup \overline{A}= \Omega$ donc $$\mu (A) + \mu(\overline{A})= \mu(\Omega)$$}
    \end{tcolorbox}
      \ex{
   ...
   $A =$ Les élèves ayant leur anniversaire en juillet, $\qquad \mu(A) = $\\
   \vspace{10pt}
   $\overline{A} =$ Les élèves n'ayant pas leur anniversaire en juillet, $\qquad \mu(\overline{A}) = $\\
   \vspace{10pt}
   $\mu (A) + \mu(\overline{A})= $}
\vspace{10pt}
\begin{tcolorbox}[colback=magenta!0, colframe=red!50!blue, title=Propriétés de l'union des ensembles quelconques]
    \prop{Si $A\subset\Omega$, et $B \subset \Omega$ des ensembles quelconques, alors $$ \mu (A\cup B) = \mu(A) + \mu(B) - \mu(A\cap B)$$}
\end{tcolorbox}
    \ex{
   $\Omega =$ Les élèves de la classe.\\
   Avec la mesure $\mu$ étant le nombre d'élèves dans l'ensemble.\\
...
\vspace{10pt}
    ...
    \vspace{0.5cm}
    Note : une des mesures les plus intuitives est le nombre d'éléments dans l'ensemble. On peut le noter $\#A$.\\
    \ex{
        $A = \{1, 3, 7, 15, 2\}$ \hspace{2cm}
        $\# A =$
    }